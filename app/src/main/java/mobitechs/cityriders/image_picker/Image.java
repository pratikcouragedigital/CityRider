package mobitechs.cityriders.image_picker;
...
import mobitechs.cityriders.Add_Rider;
import mobitechs.cityriders.Join_CityRiders;
...
public class Image {
    ...
    public Image(Add_Rider add_Rider, String activityName, OnRecyclerSetImageListener onRecyclerSetImageListener) {
        this.activity = add_Rider;
        this.activityName = activityName;
        this.onRecyclerSetImageListener = onRecyclerSetImageListener;
    }

    public Image(Join_CityRiders join_cityRiders, String joinRider, Join_CityRiders onRecyclerSetImageListener) {
        this.activity = join_cityRiders;
        this.activityName = joinRider;
        this.onRecyclerSetImageListener = onRecyclerSetImageListener;
    }
    ...
    public interface OnRecyclerSetImageListener {
        ...
        void onRecyclerImageSet(Bitmap imageToShow, byte[] firstImagePath);
    }
...
//                }
    ...
    private byte[] createBase64StringFromImageFile(File originalFile) {
    ...
    private void setBitmapToImage(Bitmap imageToShow) {
        if (imageBase64String == "") {
            ...
            byte[] imageByte = createBase64StringFromImageFile(new File(currentImagePath));
            onRecyclerSetImageListener.onRecyclerImageSet(imageToShow, imageByte);
        }
    ...
    private void imageCompress(File sourceFile) {
...
//        cropIntent.putExtra("return-data", true);
    ...
    private Bitmap saveCropBitmap(Bitmap imageToShow) {
    ...
    public class SelectGalleryImage extends AsyncTask<Void, Void, Void> {
        @Override
        protected Void doInBackground(Void... params) {
            // Create intent to Open Image applications like Event_List, Google Photos
            File photoFile = null;
            try {
                photoFile = createImageFile();
            } catch (IOException ex) {
